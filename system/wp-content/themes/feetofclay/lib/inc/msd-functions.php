...
function msdlab_get_excerpt( $post_id, $excerpt_length = 50, $trailing_character = '&hellip;' ) {
    ...
    $the_excerpt = implode( ' ', $words ) .$trailing_character. '<a class="button" href="'.get_permalink($post_id).'">Read More</a>';
    return $the_excerpt;
}
...
function section_body_class($classes) {
    ...
    if($lang != 'en'){
	    $en_parent_id = apply_filters( 'wpml_object_id', $post_data->ID, 'post', TRUE, 'en');
		$en_parent = get_post($en_parent_id);
	    $classes[] = 'partition-'.$en_parent->post_name;
    }
    return $classes;
}
...
function get_section_title(){
    global $post;
    $post_data = get_post(get_topmost_parent($post->ID));
    $section = apply_filters('the_title',$post_data->post_title);
    return $section;
}
