jQuery(document).ready(function($) {	
    ...
    $('.flipbox').matchHeight();
    $('.genesis-blog-grid .genesis-teaser .entry-title').matchHeight();
    ...
    $('.site-inner nav a').click(function(e){
        e.preventDefault();
        var ele = $($(this).attr('href'));
        var top = ele.offset().top;
        $(window).scrollTop(top - 67);
    });
    ...
    $(window).on('resize scroll', function() {
        $('.counter-number').each(function () {
            if ($(this).isInViewport()) {
                ...
                }).addClass('counter-number-counted').removeClass('counter-number')
            }
        });
    });
    ...
    if(jQuery.browser.mobile){
        $('.rollbox').each(function(){
            var onhtml = $(this).find('.rollbox-wrapper-on').text();
            var offhtml = $(this).find('.rollbox-wrapper').text().replace(onhtml,'');
            if(onhtml != offhtml){
                $(this).find('.rollbox-on').css('height','auto');
            }
        });
    }
});
